...
- **Crop and frame your image** — after picking a photo the app opens an adjust screen: drag and zoom with a preview that already shows the app frame (top bar, tabs, veil), so you see the contrast before saving
...
- **Wallpaper behind the bars** — choose whether the top bar and the tabs go translucent over the wallpaper, or stay solid as they were
...
- Fix: the strip where the Android gesture/button bar lives now takes the counters bar colour, instead of keeping the wallpaper veil and ending the screen in a surface you never picked.
- 800 unit tests passing (was 712).
