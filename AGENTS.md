...
- The reader's font picker offers **four** families, not the five the allowlist accepts: `serif`, `sans-serif`, `monospace`, `cursive`. Android aliases `fantasy` onto `serif` (`/system/etc/fonts.xml`), so a Fantasy chip would repeat Serif exactly — the allowlist keeps accepting the value because it also guards what a restored backup may carry. The families are CSS generics resolved by the platform, so the picker needs no bundled font, no asset and no CSP change (that is issue #15, still open, for real custom fonts). Each chip draws its own label in the family it selects, which makes the chip its own preview: `FontFamily.Serif/SansSerif/Monospace/Cursive` in Compose and the WebView resolve the same generic names through the same platform config, so when a device lacks a distinct mapping the chip and the reader degrade together rather than diverging.
