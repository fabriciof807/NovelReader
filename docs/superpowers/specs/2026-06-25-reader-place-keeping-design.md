...
- **Remove `ReaderState.reloadVersion` entirely.** Delete the `val reloadVersion: Int = 0` field from the `ReaderState` data class. `LaunchedEffect(state.chapter, webView)` already covers the only remaining rebuild trigger (chapter content swap). The config and bookmark changes are applied reactively in their own `LaunchedEffect`s and no longer need a version counter.
...
- **`ReaderScreen` changes:**
  ...
  - Helper function `applyConfigJs(config: ReaderConfig): String` builds the JSON payload (bgColor, textColor, accentColor, linkColor, fontFamily, fontSize, lineHeight, autoScrollSpeed) and returns a wrapped IIFE that calls `applyConfig(JSON.parse(args))`. Reuse the existing `buildJs(code, params)` helper that already lives in `ReaderScreen.kt:97`. The `onPageFinished` lambda body reads `state.config` and `state.bookmarks` directly — these are `State<T>` property delegates (from `collectAsState()`), so the values are always current at invocation time, even though the lambda instance itself is captured once in the `WebViewClient` factory closure.
...
- **Initial composition cycle (documented for the implementer).** On first composition: (1) `webView` is `null`, the chapter-swap `LaunchedEffect` fires and short-circuits (no chapter yet). (2) `onWebViewReady` sets `webView` to the instance — the keys `webView` change, so Compose cancels the previous effect and launches a new one; the new effect still short-circuits because `isPageLoaded` is `false`. (3) `loadDataWithBaseURL` runs from elsewhere (e.g., the first `loadChapter` emission) → `onPageFinished` fires → `isPageLoaded = true` → config and bookmarks are applied, scroll is restored. The `LaunchedEffect(state.config, webView)` runs twice during the first composition (once with `webView = null`, once with `webView = instance`); both invocations short-circuit, and the third effective application happens in `onPageFinished`.

- **`pendingSearchQuery` path must be preserved.** When the reader is opened from an FTS search result, `initialSearchQuery` arrives via the nav arg (`ReaderScreen.kt:81`) and is stored in `pendingSearchQuery` (`:91-92`). The current chapter-swap `LaunchedEffect` handles it at `ReaderScreen.kt:148-183`: if `pendingSearchQuery != null`, the FTS-highlight JS is evaluated (scrollToTop is implicit because `restoreRatio` is forced to `0f` at `:137`), then `pendingSearchQuery` is consumed (set to `null`). After the refactor, this same path lives in the expanded `onPageFinished` callback: if `pendingSearchQuery != null`, call `webView.evaluateJavascript("window.scrollTo(0, 0); ...")` first (scroll to top), then the FTS-highlight JS (same body as today — `TreeWalker` + `surroundContents` + `scrollTo` the match), then consume the query (`pendingSearchQuery = null`). The spec is not changing FTS highlight behavior; the existing logic moves into `onPageFinished` unchanged.
...
- **`ReaderScreen` changes:**
  ...
  - **Idempotence note:** `applyBookmarks` is called from two paths on a chapter swap — (1) `onPageFinished` after `loadDataWithBaseURL` (always) and (2) `LaunchedEffect(state.bookmarks, webView)` if `state.bookmarks` changed in the same recomposition and `isPageLoaded` is already true. The JS function is idempotent (it first strips every existing `.bookmark-indicator` and `.bookmarked` class, then re-applies from the list), so the double call is harmless. This is by design and is the only way to keep the `LaunchedEffect` reactive on subsequent bookmark changes without coupling the bookmark-apply to the chapter-swap effect.
...
- **Testing strategy (TDD):**
  ...
  - **`onPageFinished` callback (instrumented or manual):** verify that after a chapter loads, `onPageFinished` applies config (CSS vars set on `:root`), bookmarks (`.bookmark-indicator` spans present at expected indices), and scroll restore (window scrolled to the persisted ratio, or to top if `pendingSearchQuery != null`). This is the path most likely to silently regress because it crosses the JS bridge and the WebView's async load lifecycle; cover it in a Compose test if the emulator is available, otherwise manual QA.
  - **Manual QA on device:** slider drag for font size / line height / theme should not cause scroll snap; adding/removing a bookmark should not cause scroll snap; rotating the device should preserve the chapter and scroll position; opening a chapter from an FTS search result should still highlight the first match and scroll to it (the `pendingSearchQuery` path moved into `onPageFinished` but should behave identically).
